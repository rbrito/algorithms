% ALGORITHMS -- Released 16 MAR 2005
...
% License along with this document; if not, write to the
...
\title{Algorithms}
\author{Rogério Brito \\ \texttt{rbrito@ime.usp.br}\thanks{Sincere thanks
...
separately.  The \keyword{algorithmic} environment provides an environment
for describing algorithms and the \keyword{algorithm} environment provides
a ``float'' wrapper for algorithms (implemented using \keyword{algorithmic}
...
Foundation (see the file COPYING included in this package).  This package
(currently) consists of three files: \texttt{algorithm.sty},
\texttt{algorithmic.sty} and \texttt{algorithms.tex} (the source of this
document).
